...
Elm.CommentParser = Elm.CommentParser || {};
Elm.CommentParser.make = function (_elm) {
   "use strict";
   _elm.CommentParser = _elm.CommentParser || {};
   if (_elm.CommentParser.values)
   return _elm.CommentParser.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "CommentParser",
   $Basics = Elm.Basics.make(_elm),
   ...
   var createRegexString = F2(function (s,
   r) {
      return $String.isEmpty(r) ? s : A2($Basics._op["++"],
      s,
      A2($Basics._op["++"],"|",r));
   });
   var matches = $Dict.fromList(_L.fromArray([{ctor: "_Tuple2"
                                              ,_0: "&gt;"
                                              ,_1: ">"}
                                             ,{ctor: "_Tuple2"
                                              ,_0: "&lt;"
   ...
   var regex = $Regex.regex(A3($List.foldr,
   createRegexString,
   "",
   $Dict.keys(matches)));
   ...
   var toNode = function (s) {
      return _U.eq(s,
      "i") ? $Html.i : _U.eq(s,
      "b") ? $Html.b : _U.eq(s,
      "a") ? $Html.a : _U.eq(s,
      "pre") ? $Html.pre : _U.eq(s,
      "code") ? $Html.code : function () {
         var sfx = A2($Debug.log,
         "missing tag:",
         s);
         return $Html.span;
      }();
   };
   var textNode = function (p) {
      return A2($Parser._op["<$>"],
      function (s) {
         return $Html.text(A4($Regex.replace,
         $Regex.All,
         regex,
         replace,
         s));
   ...
   var attr = F2(function (t,v) {
      return _U.eq(t,
      "href") ? $Maybe.Just($Html$Attributes.href(v)) : _U.eq(t,
      "class") ? $Maybe.Just($Html$Attributes.$class(v)) : $Maybe.Nothing;
   });
   var quote = A2($Parser.or,
   $Parser.symbol(_U.chr("\"")),
   $Parser.symbol(_U.chr("\'")));
   var whitespace = A2($Parser.map,
   function (e) {
      return _U.chr(" ");
   },
   $Parser.some($Parser.symbol(_U.chr(" "))));
   var noneOf2 = F2(function (x,
   y) {
      return $Parser.satisfy(function (s) {
         return !_U.eq(s,
         x) && !_U.eq(s,y);
      });
   });
   var noneOf = function (x) {
      return $Parser.satisfy(function (s) {
         return !_U.eq(s,x);
      });
   };
   ...
   toString(noneOf(_U.chr("<"))));
   var nodeContent = function (l) {
      return A2($Parser.map,
      function (c) {
         return l(_L.fromArray([c]));
      },
      content);
   };
   var tagname = toString(A2(noneOf2,
   _U.chr(">"),
   _U.chr(" ")));
   var closingTag = A2($Parser._op["<*"],
   A2($Parser._op["*>"],
   $Parser.token($String.toList("</")),
   tagname),
   $Parser.symbol(_U.chr(">")));
   var attributeName = A2($Parser._op["<*"],
   A2($Parser._op["<*"],
   A2($Parser._op["*>"],
   whitespace,
   toString(noneOf(_U.chr("=")))),
   $Parser.symbol(_U.chr("="))),
   quote);
   var attributeValue = toString(A2(noneOf2,
   _U.chr("\""),
   _U.chr("\'")));
   var toAttribute = function (s) {
      return A2($Parser.map,
      attr(s),
      attributeValue);
   };
   var attribute = A2($Parser._op["<*"],
   A2($Parser.andThen,
   attributeName,
   toAttribute),
   quote);
   var keepJust = function (m) {
      return function () {
         switch (m.ctor)
   ...
   var mapAndFilterAttributes = function (l) {
      return A2($List.map,
      function (e) {
         return function () {
            switch (e.ctor)
            {case "Just": return e._0;}
            _U.badCase($moduleName,
            "on line 95, column 43 to 62");
         }();
      },
      A2($List.filter,keepJust,l));
   };
   var withAttribute = function (e) {
      return A2($Parser.map,
      e,
      A2($Parser._op["<$>"],
      mapAndFilterAttributes,
      $Parser.many(attribute)));
   };
   var openingTag = A2($Parser._op["<*"],
   A2($Parser.andThen,
   A2($Parser._op["*>"],
   $Parser.symbol(_U.chr("<")),
   A2($Parser._op["<$>"],
   ...
   var parseP = function (p) {
      return function () {
         var result = A2($Parser.parseString,
         A2($Parser._op["<$>"],
         toHtml,
         $Parser.some(A2($Parser._op["<|>"],
         tag,
         content))),
         p);
         return function () {
   ...
   var parse = function (s) {
      return function () {
         var s1 = A4($Regex.replace,
         $Regex.All,
         $Regex.regex("<pre><code>"),
         function (_v9) {
            return function () {
               return "<pre>";
            }();
         ...
         s1);
         var splited = A2($String.split,
         "<p>",
         s2);
         return _L.fromArray([A2($Html.div,
         _L.fromArray([]),
         A2($List.map,parseP,splited))]);
      }();
   };
   _elm.CommentParser.values = {_op: _op
                               ,keepJust: keepJust
                               ,toString: toString
                               ,noneOf: noneOf
                               ,noneOf2: noneOf2
                               ...
                               ,mapAndFilterAttributes: mapAndFilterAttributes
...
Elm.HN = Elm.HN || {};
Elm.HN.make = function (_elm) {
   "use strict";
   _elm.HN = _elm.HN || {};
   if (_elm.HN.values)
   return _elm.HN.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $moduleName = "HN",
   $Basics = Elm.Basics.make(_elm),
   $CommentParser = Elm.CommentParser.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $RelativeTime = Elm.RelativeTime.make(_elm),
   $Result = Elm.Result.make(_elm),
         ...
         {case "Comment":
            return A2($Html.li,
              ...
              _L.fromArray([A2($Html.div,
                           _L.fromArray([]),
                           $CommentParser.parse(A2($Debug.log,
                           "text",
                           comment._0.text)))
                           ,A2($Html.ul,
                           _L.fromArray([]),
   ...
   var renderUser = function (user) {
      ...
      _L.fromArray([A2($Html.dt,
                   ...
                   ,A2($Html.dd,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text(user.id)]))
                   ,A2($Html.dt,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("About")]))
                   ,A2($Html.dd,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text(user.about)]))]));
   };
   var renderNews = F2(function (time,
   news) {
      return function () {
         var t = $String.isEmpty(news.url) ? "" : "_blank";
         var linkToComments = A2($Basics._op["++"],
         "#!item/",
         $Basics.toString(news.id));
         var url = $String.isEmpty(news.url) ? linkToComments : news.url;
         return A2($Html.li,
         _L.fromArray([$Html$Attributes.key($Basics.toString(news.id))]),
         _L.fromArray([A2($Html.div,
                      _L.fromArray([]),
                      _L.fromArray([A2($Html.h3,
                                   ...
                                   _L.fromArray([$Html$Attributes.href(url)
                                                ,$Html$Attributes.target(t)]),
                                   _L.fromArray([$Html.text(news.title)]))]))
                                   ,A2($Html.p,
                                   _L.fromArray([]),
                                   _L.fromArray([A2($Html.span,
                                                ...
                                                "#!user/",
                                                news.by))]),
                                                _L.fromArray([$Html.text(news.by)]))]))]))
                      ,A2($Html.a,
                      _L.fromArray([$Html$Attributes.href(linkToComments)]),
                      _L.fromArray([A2($Html.span,
                                   _L.fromArray([$Html$Attributes.$class("score main")]),
                                   _L.fromArray([$Html.text($Basics.toString(news.score))]))
                                   ,A2($Html.span,
                                   _L.fromArray([$Html$Attributes.$class("comment-count main")]),
                                   _L.fromArray([$Html.text($Basics.toString(news.comments))]))]))]));
      }();
   });
   var render = function (state) {
      return function () {
         var content = function () {
            var _v2 = state.content;
            switch (_v2.ctor)
            {case "Just":
               return function () {
                    switch (_v2._0.ctor)
                    {case "Comment\'\'":
                       return function () {
                            ...
                            {case "Just":
                               return A2($Html.ul,
                                 _L.fromArray([]),
                                 _L.fromArray([renderComments(_v7._0)]));
                               case "Nothing":
                               return A2($Html.div,
                                 _L.fromArray([]),
                                 _L.fromArray([]));}
         ...
         _L.fromArray([A2($Html.div,
                      _L.fromArray([$Html$Attributes.$class("column sidebar")]),
                      _L.fromArray([A2($Html.ul,
                      _L.fromArray([]),
                      A2($List.map,
                      renderNews(state.time),
                      state.news))]))
                      ,A2($Html.div,
                      _L.fromArray([$Html$Attributes.$class("column main content")]),
            ...
            return function () {
                 ...
                 {case "Comment\'":
                    return _U.replace([["content"
                                       ,$Maybe.Just(Comment$$(_v9._1._0))]
                                      ,["time",_v9._0]],
                      state);
                    case "News\'":
                    return _U.replace([["news"
                                       ,_v9._1._0]
                                      ,["time",_v9._0]],
                      state);
                    case "User\'":
                    return _U.replace([["content"
                                       ,$Maybe.Just(User$$(_v9._1._0))]
                                      ,["time",_v9._0]],
                      state);}
                 _U.badCase($moduleName,
   ...
   var LoadingContent$ = function (a) {
      return {ctor: "LoadingContent\'"
             ,_0: a};
   };
   var Comment$ = function (a) {
      return {ctor: "Comment\'"
   ...
   var commentsDecoder = A4($Json$Decode.object3,
   toComment,
   ...
   A2($Json$Decode._op[":="],
   "childs",
   $Json$Decode.list(lazy(function (_v17) {
      return function () {
         return commentsDecoder;
      }();
   }))));
   var decodeComments = function (s) {
      return $Result.toMaybe(A2($Debug.log,
      "decode",
      A2($Json$Decode.decodeValue,
      commentsDecoder,
      s)));
   };
   var News = F8(function (a,
   b,
   c,
   d,
   e,
   ...
   var comments = _P.portIn("comments",
   _P.incomingSignal(function (v) {
      return v;
   }));
   var user = _P.portIn("user",
   _P.incomingSignal(function (v) {
      return typeof v === "object" && "id" in v && "about" in v ? {_: {}
                                                                  ...
                                                                  v.about)} : _U.badPort("an object with fields \'id\', \'about\'",
      v);
   }));
   var topNews = _P.portIn("topNews",
   _P.incomingSignal(function (v) {
      return typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "id" in v && "title" in v && "by" in v && "url" in v && "text" in v && "time" in v && "score" in v && "comments" in v ? {_: {}
                                                                                                                                                                 ...
                                                                                                                                                                 v.title)
                                                                                                                                                                 ,by: typeof v.by === "string" || typeof v.by === "object" && v.by instanceof String ? v.by : _U.badPort("a string",
                                                                                                                                                                 v.by)
                                                                                                                                                                 ...
                                                                                                                                                                 v.time)
                                                                                                                                                                 ,score: typeof v.score === "number" ? v.score : _U.badPort("a number",
                                                                                                                                                                 v.score)
                                                                                                                                                                 ,comments: typeof v.comments === "number" ? v.comments : _U.badPort("a number",
                                                                                                                                                                 v.comments)} : _U.badPort("an object with fields \'id\', \'title\', \'by\', \'url\', \'text\', \'time\', \'score\', \'comments\'",
         v);
      })) : _U.badPort("an array",v);
   }));
   var updates = $Time.timestamp($Signal.mergeMany(_L.fromArray([A2($Signal._op["<~"],
                                                                News$,
                                                                topNews)
                                                                ,A2($Signal._op["<~"],
                                                                User$,
   ...
   _elm.HN.values = {_op: _op
                    ...
                    ,State: State
                    ,toComment: toComment
                    ,commentsDecoder: commentsDecoder
                    ,lazy: lazy
...
Elm.Parser.make = function (_elm) {
   ...
   f) {
      return function ($) {
         return $List.concat($List.map(function (_v0) {
            return function () {
               switch (_v0.ctor)
               {case "_Tuple2": return A2(f,
                    _v0._0,
   ...
   var and = F2(function (p,q) {
      return function ($) {
         return $List.concat($List.map(function (_v4) {
            return function () {
               switch (_v4.ctor)
               {case "_Tuple2":
                       ...
                       return function () {
                          ...
                          {case "_Tuple2":
                             return {ctor: "_Tuple2"
                                    ...
                                    ,_1: _v8._1};}
                          _U.badCase($moduleName,
                          "on line 107, column 60 to 67");
                       }();
                    })(q(_v4._1));}
               _U.badCase($moduleName,
               "on line 107, column 37 to 77");
            }();
         })(p($)));
      };
   });
   _op["<*>"] = and;
   var or = F3(function (p,q,xs) {
   ...
   var empty = $Basics.always(_L.fromArray([]));
   var choice = A2($List.foldr,
   or,
   empty);
   var satisfy = F2(function (p,
   xs) {
      return function () {
         switch (xs.ctor)
         {case "::":
            return p(xs._0) ? _L.fromArray([{ctor: "_Tuple2"
                                            ,_0: xs._0
                                            ,_1: xs._1}]) : _L.fromArray([]);
            case "[]":
            return _L.fromArray([]);}
         _U.badCase($moduleName,
   ...
   var many = F2(function (p,xs) {
      return function () {
         var _v22 = p(xs);
         switch (_v22.ctor)
         {case "[]": return A2(succeed,
              _L.fromArray([]),
              xs);}
         return A2(and,
         A2(map,
   ...
   var parseString = function (p) {
      return function ($) {
         return parse(p)($String.toList($));
      };
   };
...
Elm.Parser.Char.make = function (_elm) {
   ...
   A2($Parser.optional,
   A2($Parser.map,
   $Basics.always(function (x) {
      return 0 - x;
   }),
   $Parser.symbol(_U.chr("-"))),
   $Basics.identity),
   natural);
   var $float = A2($Parser.and,
   A2($Parser._op["<*"],
   A2($Parser.map,
   F2(function (i,f) {
      return $Basics.toFloat(i) + 0.1 * A3($List.foldr,
      F2(function (b,a) {
         return a / 10 + b;
      }),
      0,
...
Elm.RelativeTime = Elm.RelativeTime || {};
Elm.RelativeTime.make = function (_elm) {
   "use strict";
   _elm.RelativeTime = _elm.RelativeTime || {};
   if (_elm.RelativeTime.values)
   return _elm.RelativeTime.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "RelativeTime",
   $Basics = Elm.Basics.make(_elm),
   $Time = Elm.Time.make(_elm);
   var time = F3(function (s,p,t) {
      return _U.eq(t,
      "1") ? A2($Basics._op["++"],
      t,
      A2($Basics._op["++"],
   ...
   var timespanAsString = F2(function (t,
   f) {
      return $Basics.toString($Basics.floor(t / f));
   });
   var minute = 60;
   var hour = minute * 60;
   var day = hour * 24;
   var relativeTime = F2(function (start,
   end) {
      return function () {
         var timespan = $Time.inSeconds(end) - $Basics.toFloat(start);
         return _U.cmp(timespan,
         minute) < 0 ? A2(time,
         "second",
         "seconds")($Basics.toString(timespan)) : _U.cmp(timespan,
         hour) < 0 ? A2(time,
         "minute",
         "minutes")(A2(timespanAsString,
         timespan,
         minute)) : _U.cmp(timespan,
         day) < 0 ? A2(time,
         "hour",
         "hours")(A2(timespanAsString,
         timespan,
         hour)) : _U.cmp(timespan,
         day) > -1 ? A2(time,
         "day",
         "days")(A2(timespanAsString,
         timespan,
         day)) : _U.badIf($moduleName,
         "between lines 21 and 24");
      }();
   });
   _elm.RelativeTime.values = {_op: _op
                              ,minute: minute
                              ,hour: hour
                              ,day: day
                              ,timespanAsString: timespanAsString
                              ,time: time
